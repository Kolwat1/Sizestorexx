...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_SESSION['id'])) {
        ...
        $plr1 = dd_q("SELECT * FROM users WHERE id = ?", [$_SESSION['id']])->fetch(PDO::FETCH_ASSOC);
            ...
            $p = dd_q("SELECT * FROM sausageman WHERE id = ?", [$id]);
            ...
            if ($p !== false && $p->rowCount() == 1) {
                ...
                if ($plr['status'] == 'เปิดขาย') {
                    ...
                    if ($plr1['point'] >= $amountToDeduct) {
                        ...
                        $apiUrl = 'https://www.khanthep.in.th/api/v1/termgame/sausageman';
                        $apiData = array(
                            'BuyId' => $plr['number'],
                            'Ref1' => $userid,
                            'Ref2' => 'NO_SERVER',
                        ...
                        if ($statusCode == 200) {
                            ...
                            dd_return(true, "เติมตรง Sausage Man เข้าไอดี {$userid} ราคา {$plr['amount']} บาท สำเร็จ (รอ 1 - 15 นาที) !!");
                        } else {
